...
public:
    ...
    /**
     * @brief Update column obstacles.
     *
     * @param idx - The index of the column obstacle.
     * @param r - The radius of the column obstacle.
     * @param x - The position of the column obstacle.
     */
    void updateColumn(int idx, Real r, vec3r x){
        // Check if the index is valid
        if(idx<0 || idx>3) return;

        // Update the radius if it is positive
        if(r>0) m_params.ocol_r[idx]=r;

        // Update the position if it has a non-zero length
        if(length(x)>0) m_params.ocol_x[idx]=x;
    }

    /**
     * @brief Update the sphere obstacles
     * 
     * @param idx The index of the sphere obstacle (0-3)
     * @param r The radius of the sphere obstacle
     * @param x The position of the sphere obstacle
     */
    ...
    /**
     * @brief Updates the parameters on the device.
     *
     * @details This function submits the parameters to the device for updating.
     *
     * @param params A pointer to the parameters to be updated.
     */
...
public:
    ...
    /**
     * @brief Get the number of foam particles.
     *
     * @return The number of foam particles.
     */
