...
FluidSystem::FluidSystem(uint numParticles, Real radius, uint3 gridSize, vec3r worldMin, vec3r worldMax, bool useFoam, bool useSurfaceTension) :
	...
	m_params.rho0[(int)PhaseType::Sand]=1000.f;
...
void FluidSystem::_initialize(){
	...
	m_hx.resize(m_numParticles, false);
	m_hx.fill(m_numParticles, make_vec3r(0.0f));

	m_hv.resize(m_numParticles, false);
	m_hm.resize(m_numParticles, false);
	m_hp.resize(m_numParticles, false);
	m_hc.resize(m_numParticles, false);
	m_hv.fill(m_numParticles, make_vec4r(0.0f));
	m_hm.fill(m_numParticles, 0.0f);
	m_hp.fill(m_numParticles, 0);
	m_hc.fill(m_numParticles, make_vec3r(0.5f,0.5f,0.5f));

	m_hRigidParticleSign.resize(m_numParticles, false);
	m_hRigidParticleSign.fill(m_numParticles, 0);

	//// allocate device storage
	m_dConstraintStart.resize(m_numParticles, false);
	m_dConstraintCnt.resize(m_numParticles, false);
	m_dConstraintParticleMap.resize(m_numParticles, false);

	m_dr.resize(m_numParticles*3, false);
	m_dq.resize(m_numParticles, false);
	
	m_dCellStart.resize(m_numCells, false);
	m_dCellEnd.resize(m_numCells, false);
	// m_dNbrs.resize(m_numCells, false);
...
FluidSystem::update(float deltaTime){
	...
	for(uint s = 0; s < m_subSteps; s++) {
		...
		for (uint i = 0; i < m_solverIterations; i++){
			...
			// VecArray<vec3r, MemType::CPU> debug_vec3rs;
			// debug_vec3rs.resize(m_numParticles);
			// copyArray<vec3r, MemType::CPU, MemType::GPU>(&debug_vec3rs.m_data, &pf.getTempPositionRef().m_data, 0, m_curNumParticles);

			// LOG_OSTREAM_DEBUG<<"print dx "<<m_params.density0<<std::endl;
			// for(int i=0;i<m_curNumParticles;i+=10){
			// 	LOG_OSTREAM_DEBUG<<"\t"<<i<<" "<<debug_vec3rs[i].x<<","<<debug_vec3rs[i].y<<","<<debug_vec3rs[i].z<<std::endl;
			// }
